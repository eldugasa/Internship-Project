...
export const calculateStats = (users, projects, teams, tasks) => {
  ...
  const userRoles = {
    ...
    projectManagers: users.filter(u => u.role === 'project-manager' || u.role === 'project_manager').length,
    teamMembers: users.filter(u => u.role === 'team-member' || u.role === 'team_member').length
  };
