...
const QATesterDashboard = () => {
  ...
  const completedTasks = tasks.filter((task) =>
    ["passed", "completed", "done"].includes(
  ...
  const pendingRetest = tasks.filter(
    (t) => t.status === "pending-retest",
  ).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {statCards.map((stat, index) => (
          ...
          >
            <div
              className={`w-12 h-12 rounded-lg ${stat.bg} ${stat.color} flex items-center justify-center mb-4`}
            >
