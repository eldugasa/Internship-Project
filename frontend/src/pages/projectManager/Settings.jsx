...
import { updateCurrentUserProfile, updateCurrentUserPassword } from '../../services/usersService';
...
const SettingsPage = () => {
  ...
  if (!loaderData) {
    return <SettingsError error="No data received from server" onRetry={handleRetry} />;
  }

  return (
    <Suspense fallback={<SettingsSkeleton />}>
      <Await 
        resolve={Promise.all([loaderData.profile, loaderData.notifications])}
        errorElement={<SettingsError error="Failed to load settings data" onRetry={handleRetry} />}
      >
        {([profileData, notificationsData]) => {
          // Process profile data
          const profile = {
            id: profileData.id,
            name: profileData.name || '',
            email: profileData.email || '',
            role: profileData.role || 'project-manager',
            phone: profileData.phone || '',
            location: profileData.location || '',
            team: profileData.team?.name || 'Engineering Team',
            joinDate: profileData.createdAt || new Date().toISOString(),
            profileImage: profileData.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(profileData.name || 'User')}&background=0f5841&color=fff&size=128`
          };
          ...
          const resetToDefault = async () => {
            try {
              setPrefsSaving(true);
              await apiClient('/notification-prefs/reset', {
                method: 'POST'
              });
