...
const Reports = () => {
  ...
  if (!loaderData) {
    return <ReportsError error="No data received from server" onRetry={handleRetry} />;
  }

  // Define reports here to access filteredData
  const getReports = (filteredData, stats, teamPerformance) => [
    {
      id: 'progress',
    ...
    },
    {
      ...
      getDetails: () => {
        const priorityCounts = {
          high: filteredData.tasks.filter(t => t.priority === 'high').length,
          ...
          .map(t => {
            ...
            if (t.dueDate) {
              ...
              if (dueDateObj && !isNaN(dueDateObj.getTime())) {
                formattedDate = dueDateObj.toLocaleDateString('en-GB');
              } else {
  ...
  return (
    <Suspense fallback={<ReportsSkeleton />}>
      <Await 
        ...
        errorElement={<ReportsError error="Failed to load reports data" onRetry={handleRetry} />}
      >
        {([projects, tasks, teams]) => {
          ...
          const filteredData = filterDataByDate(safeProjects, safeTasks, dateRange);
          
          const reports = getReports(filteredData, stats, teamPerformance);
            ...
            if (data.details && data.details.length > 0) {
              ...
              if (data.reportType === 'progress') {
                ...
                bodyData = data.details.map(item => [
                  item.name || '-', item.team || '-', item.status || '-', `${item.progress || 0}%`
                ]);
              } else if (data.reportType === 'workload') {
                headers = ['Team', 'Lead', 'Members', 'Projects', 'Completion'];
                bodyData = data.details.map(item => [
                  item.name || '-', item.lead || '-', (item.members || 0).toString(),
                  (item.projects || 0).toString(), `${item.completionRate || 0}%`
                ]);
              } else if (data.reportType === 'deadlines') {
                headers = ['Task', 'Project', 'Due Date', 'Status', 'Priority'];
                bodyData = data.details.map(item => [
                  item.name || '-', item.project || '-', item.dueDate || '-', item.status || '-', item.priority || '-'
                ]);
              } else {
                headers = ['Item', 'Value'];
                bodyData = data.details.map(item => [
                  item.name || item.label || '-', `${item.value || item.progress || item.completionRate || 0}%`
                ]);
              }
          ...
          return (
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={generateReport} className="px-4 py-2 bg-[#4DA5AD] text-white rounded-lg hover:bg-[#3D8B93] flex items-center">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#4DA5AD] focus:border-transparent" />
                      </div>
                      <div>
                        <label className="block text-sm text-gray-600 mb-1">End Date</label>
                        <input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#4DA5AD] focus:border-transparent" />
                      </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="space-y-2">
                      {reports.map(report => (
                        <button key={report.id} onClick={() => setSelectedReport(report.id)} className={`w-full text-left p-3 rounded-lg transition ${selectedReport === report.id ? 'bg-[#4DA5AD]/10 border border-[#4DA5AD]' : 'border border-gray-200 hover:bg-gray-50'}`}>
                          <div className="flex items-center">
                            <div className={`p-2 rounded-lg mr-3 ${selectedReport === report.id ? 'bg-[#4DA5AD] text-white' : 'bg-gray-100 text-gray-600'}`}>{report.icon}</div>
                            <div><div className="font-medium text-gray-900">{report.name}</div><div className="text-xs text-gray-500">{report.description}</div></div>
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                <div className="lg:col-span-2">
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div className="flex justify-between items-center mb-6">
                      <div><h2 className="text-xl font-bold text-gray-900">{currentReport?.name}</h2><p className="text-gray-600">Generated: {new Date().toLocaleDateString()}</p></div>
                      <div className="flex space-x-2">
                        {['PDF', 'Excel', 'CSV'].map(format => (
                          <button key={format} onClick={() => handleDownload(format)} className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 flex items-center text-sm">
                        ...
                        <div className="space-y-3 max-h-80 overflow-y-auto">
                          {details.map((item, i) => (
                            <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                              <span className="font-medium text-gray-700">{item.name || item.label || item.title}</span>
