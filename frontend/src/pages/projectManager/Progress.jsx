...
const Progress = () => {
  ...
  if (hasError && (!loaderData?.projects || !loaderData?.tasks)) {
    return <ProgressError error={projectsError || tasksError} onRetry={handleRetry} />;
  }
  ...
  const activeProjects = projectTimeline.filter(p => 
      ...
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
                  ...
                  <div key={project.id} className="space-y-2 p-3 border border-gray-100 rounded-lg hover:bg-gray-50 transition">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-[#0f5841]">{project.progress}%</span>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div
                        className={`h-2.5 rounded-full transition-all duration-500 ${
                          project.overdue && project.status !== 'completed'
                            ? 'bg-red-500'
                            : 'bg-gradient-to-r from-[#0f5841] to-[#194f87]'
                        }`}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2">
            <Users className="w-5 h-5 text-[#0f5841]" />
              ...
              memberWorkload.map(member => (
                <div key={member.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                  <div className="flex items-center mb-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-[#0f5841] to-[#194f87] rounded-full flex items-center justify-center text-white font-medium mr-3">
                      {member.avatar}
                    </div>
                    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-xl flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-gray-900">
                {teamPerformance[0]?.teamName || 'N/A'}
              </p>
            </div>
          </div>
